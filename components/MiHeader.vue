    ...
    <div class="header__navwrap">
      <div class="header__navbar">
        <div class="header__navbar-logo">
          <NuxtLink to="/">
            <slot name="logo">
              <span class="header__navbar-logo-text">miHoYo</span>
            </slot>
          </NuxtLink>
          <!-- <a href="/">
          </a> -->
...
function linkSelect(index: number): void {
  state.value = [];
  for (const _i in props.links) {
...
onMounted(() => {
  linkSelect(useState<number>("pageIndex").value)
  for (const _i in props.links) {
    state.value.push(false);
  }
});
...
const current_page = useRoute().query.page;
function isNotIndexPge(p: unknown): boolean {
  if (p != undefined) {
    return true;
  } else {
    return false;
