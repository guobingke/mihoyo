...
const yearsEvents = ref<any[]>([]);
let navActiveIndex = 0;
const activeNav = (): HTMLElement | null => {
...
onMounted(() => {
  ...
  nextTick(() => {
    ...
    for (let i = 1; i < yearsEvents.value.length; i++) {
      transX -= (document.querySelector(`.index-${i}`) as HTMLElement).offsetWidth;
    }
...
const style = ref<string>(`transform:translate(${transX}px)`);
const disl = ref<boolean>(false);
const disr = ref<boolean>(true);

function navChange(num: number): void {
  if (num == 1) {
    swiper!.slideNext();
    navTo(swiper!.activeIndex);
  }
...
// 底部导航栏移动方法
function navTo(num: number): void {
