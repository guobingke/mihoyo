...
const page = "about";
const aboutcates = ["关于米哈游", "发展历程", "荣誉资质"];
const component = ref<Component | string>("")
const res = ref<Record<string, any> | null>(null)

onMounted(async () => {
  // 生产模式
  const url = "https://vkceyugu.cdn.bspapp.com/VKCEYUGU-b02b7f48-0227-438f-8c3a-3a5f94afa017/68d7f4e4-fa61-4cb6-8784-5e4a3eba89b2.json"
  void url
  const { data } = await useFetch<Record<string, any>>("/data/mihoyo.json", {
...
function cateHover(index: number): void {
  activeIndex.value = index;
  cateActive.value[index] = "transform: translateY(-0.3rem); transition-duration: 300ms";
}
function cateReset(index: number | null, is: boolean = false): void {
...
function cateChange(index: number): void {
...
// 切换组件路由的方法
function cateRoute(index: number): void {
