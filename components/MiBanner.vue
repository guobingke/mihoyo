...
const emit = defineEmits<{
  (e: "bannerchange" | "bannertransition", value: number): void;
}>();
...
const boardstyle = ref<string | null>(null);

function goblank(url: string): void {
...
function slideWithMouse(e: MouseEvent): void {
  emit("bannertransition", transition_duration);
  // 让元素随着鼠标相反的方向移动
  translate = -0.05 * e.pageX + 38.4; // box中的轮播大图最终位移位置
  const trans_board = -0.015 * e.pageX + 11.52; // board中背景小图的最终位移位置
...
function showVideo(videoSrc: string): void {
