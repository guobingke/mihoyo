...
type HonorYear = { y: string; list: any[] };

const props = defineProps<{ res: Record<string, any> }>()
const yearsHonor: HonorYear[] = props.res.yearsHonor
const dataTrans = ref<string>('')
let translate = 0

// 分页逻辑
const pageCount = 8         // 一页展示的数据条目
let pageIndex = 1         // 当前所在页码
// 预处理 用于 v-for
const data: { y: string; list: any[][] }[] = []
yearsHonor.forEach((item) => {
  const origin = [...item.list]
  const list: any[][] = []
  const count = Math.ceil(origin.length / pageCount)  // 不要把Math直接放在for的判断中，因为一次循环后 Math 会重新计算结果，导致循环次数变了
  for (let i = 0; i < count; i++) {
    list.push(origin.splice(0, 8))
  }
  data.push({ y: item.y, list })
})
const showList = ref<any[][]>(data[0].list)  // 当前展示荣誉数据的列表,初始化默认第一个
// 换页的方法
function next(): void {
  const dom = document.querySelector(".about-honor-container__data") as HTMLElement
...
const lineTrans = ref<string>('')
let translateY = 0
let time1: ReturnType<typeof setTimeout> | number = 0
let time2: ReturnType<typeof setTimeout> | number = 0
function navTo(index: number): void {
  clearTimeout(time1)
  clearTimeout(time2)
  translateY = 0
  for (let i = 0; i < index - 0; i++) {
    translateY += (document.querySelector('.about-honor-swiper .swiper-slide') as HTMLElement).clientHeight
  }
  lineTrans.value = `transform:translateY(${translateY}px)`
  const dom = document.querySelector('.about-honor-container__content') as HTMLElement
...
onMounted(() => {
  const swiper = new Swiper(".about-honor-swiper", {
    direction: 'vertical',
    freeMode: {
      enabled: false,
      sticky: true,
