...
function getSlidePage(slide: Element | undefined) {
  const firstChild = slide?.children?.[0] as HTMLElement | undefined
  const className = firstChild?.classList?.[0]
  return className?.startsWith("home-") ? className.replace("home-", "") : ""
}
  ...
  mySwiper = new Swiper(".home-swiper", {
    ...
    on: {
      ...
      slideChangeTransitionStart: () => {
        // 滑动前修改路由和 url
        if (!mySwiper) return
        let page = getSlidePage(mySwiper.slides[mySwiper.activeIndex]); // 当前活跃页
        if (!page) return
...
function routeMatch(speed = 0) {
  ...
  if (query.page == undefined || query.page == "/" || query.page == "") {
    ...
    let targetIndex = 0
    Array.from(mySwiper.slides).forEach((slide, index) => {
      const page = getSlidePage(slide)
      if (page === query.page) {
        targetIndex = index
      }
    })
    mySwiper.slideTo(targetIndex, speed);
  }
}
...
watch(() => useState<number>('newsSwiperProgress').value, (v = 0) => {
  if (!mySwiper) return
  const asideSlogan = document.querySelector(".aside-slogan")
  const asideScroll = document.querySelector(".aside-scroll")
  if (v.toFixed(0) == -1) {
    asideSlogan?.classList.add("aside-fade")
    asideScroll?.classList.add("aside-fade")
    mySwiper.mousewheel.disable();
  } else {
    asideSlogan?.classList.remove("aside-fade")
    asideScroll?.classList.remove("aside-fade")
    setTimeout(() => {
