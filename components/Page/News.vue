              ...
              <div class="home-news__content">
                <div class="swiper-wrapper">
                  <div class="swiper-slide" v-for="item in newsList" :key="item.id">
                    <a :href="getNewsHref(item)" class="home-news-view">
                      <div class="home-news-view__info">
                        <div class="home-news-view__info-date">
                          <div>
                            {{ getDateParts(item.date).month }}
                            <span class="slash">/</span>
                            {{ getDateParts(item.date).day }}
                          </div>
                          <div class="year">{{ getDateParts(item.date).year }}</div>
                        </div>
                ...
                <div class="home-news-nav swiper-no-swiping">
                  <div class="home-news-nav__item" :class="navIndex == index ? 'home-news-nav__item--active' : ''"
                    v-for="(item, index) in newsList" :key="item.id" @click="navChange(index)">
                    <span>{{ index + 1 }}</span>
                  </div>
                </div>
              </div>
...
const props = withDefaults(defineProps<{
  newsRes?: NewsItem[] | null;
}>(), {
  newsRes: () => [],
})

const newsList = computed(() => props.newsRes || [])
const navIndex = ref(0)
const localText = useState("localText")
const newsSwiperProgress = useState<number>("newsSwiperProgress", () => 0)

let swiper: Swiper | null = null
let contentSwiper: Swiper | null = null

function getNewsHref(item: NewsItem) {
  return `https://www.mihoyo.com/news/${item.id}`
}
...
function wheel() {
  const progress = swiper?.slides?.[1]?.progress
  if (typeof progress === "number") {
    newsSwiperProgress.value = progress
  }
}
...
function bgAct(e: MouseEvent) {
  const image = document.querySelector<HTMLElement>(".home-news__content .swiper-slide-active .home-news-view__banner-bg-img")
  if (!image) return
  image.style.transform = `translate(${(-39.35 / window.innerWidth) * e.pageX + 20}px,${(-20.46 / window.innerHeight) * e.pageY + 9.36}px)`
}

function initLastSwiper() {
  swiper = new Swiper(".last-swiper", {
    direction: "vertical",
...
onMounted(() => {
  initLastSwiper()
  contentSwiper = new Swiper(".home-news__content", {
    observer: true, // 修改swiper自己或子元素时，自动初始化swiper,也就是响应式
    observeParents: true, // 修改swiper的父元素时，自动初始化swiper
    ...
    on: {
      transitionStart: () => {
        navIndex.value = contentSwiper?.activeIndex || 0
      }
    }
  })
  setTimeout(() => {
