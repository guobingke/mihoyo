  ...
  <div :class="'home-' + page" class="fp-page" @mousemove="slideWithMouse">
    ...
    <div class="wrapper">
      <div class="banner">
        <div class="banner__role">
          <img ref="roleOne" src="/img/char1.e4264de.png" class="banner__role-1" alt>
          <img ref="roleTwo" src="/img/char2.04c821e.png" class="banner__role-2" alt>
        </div>
    ...
    <div class="fp-page__wrap" v-if="ready">
      <div class="home-join-tabs">
        <div class="home-join-tab" :class="{ 'home-join-tab--active': activeTab === 'jobs' }" @click="activeTab = 'jobs'">
          {{ localText.jobTab }}
        </div>
        <div class="home-join-tab" :class="{ 'home-join-tab--active': activeTab === 'values' }" @click="activeTab = 'values'">
          {{ localText.valuesTab }}
        </div>
        <span class="line" :style="lineTrans"></span>
      </div>
      <div class="join-jobs home-join-container" :class="{ none: activeTab !== 'jobs' }">
        <div class="join-jobs-tabs">
          <div class="join-jobs-tabs-wrap">
            <div class="join-jobs-tab join-jobs-tab--active">
              <div class="span-wrapper" v-for="it in [1, 2, 3]" :key="it">
          ...
          <div class="join-jobs-tabs-wrap" @mouseenter="tabHover" @mouseleave="tabReset"
            ...
            <div class="join-jobs-tab">
              <div class="span-wrapper" v-for="it in 3" :key="it">
        ...
        <div class="join-jobs-list">
          <div class="join-jobs-list-item" v-for="item in jobs" :key="item.id">
            ...
            <span v-if="item.jobCount !== undefined && item.jobCount !== ''" class="join-jobs-list-count">{{ item.jobCount }}</span>
          </div>
        </div>
        <div class="more-btn" @click="goTo('https://jobs.mihoyo.com/#/jobs')">{{ localText.allJobsBtn }}</div>
      </div>
      <div class="join-weal home-join-container" :class="{ none: activeTab !== 'values', 'join-weal--active': activeTab === 'values' }">
...
const props = withDefaults(defineProps<{
  jobsList?: JobsPayload | null;
  valuesRes?: ValueItem[] | null;
}>(), {
  jobsList: null,
  valuesRes: () => [],
})

const localTextState = useState<Record<string, any>>("localText")
const localText = computed(() => localTextState.value || {})
const ready = computed(() => Object.keys(localText.value).length > 0)
const activeTab = ref<"jobs" | "values">("jobs")
const roleOne = ref<HTMLImageElement | null>(null)
const roleTwo = ref<HTMLImageElement | null>(null)

const fallbackJobIds = [22499, 22500, 22501, 22502, 22504, 22505, 71699, 84174]
const jobs = computed<Job[]>(() => {
  const source = props.jobsList?.jobs
  const remoteJobs = Array.isArray(source) ? source : source ? Object.values(source) : []

  if (remoteJobs.length > 0) {
    return remoteJobs
  }

  return fallbackJobIds.map((id) => ({
    id,
    label: localText.value[`moka-label-${id}`] || "",
    jobCount: "",
  })).filter((item) => item.label)
})

let timeTask: ReturnType<typeof setTimeout> | null = null
function tabHover({ currentTarget }: MouseEvent) {
  const tab = (currentTarget as HTMLElement).firstElementChild as HTMLElement | null
  if (!tab) return
  tab.style.transitionDuration = "300ms"
  tab.style.transform = "translateY(-0.3rem)"
}
function tabReset({ currentTarget }: MouseEvent) {
  const tab = (currentTarget as HTMLElement).firstElementChild as HTMLElement | null
  if (!tab) return
  if (timeTask) clearTimeout(timeTask)
  tab.style.transform = "translateY(0rem)"
  timeTask = setTimeout(() => {
    tab.style.transitionDuration = "0ms"
    tab.style.transform = "translateY(-0.6rem)"
  }, 300)
}
function goTo(url) { window.open(url, "_blank") }   // 无关

const lineTrans = computed(() => activeTab.value === "values" ? "transform: translateY(1.04rem)" : "")
function hasContent(item: ValueItem) {
  return item.cards?.some((card) => card.content !== "") || false
}
function slideWithMouse(e: MouseEvent) {
  // 角色图片随鼠标移动
  // role1   0.22rem  ~  -0.22remz
  // y = (-0.44 / window.innerWidth) * e.pageX + 0.22
  if (roleOne.value) {
    roleOne.value.style.transform = `translateX(${(-0.44 / window.innerWidth) * e.pageX + 0.22}rem) scale(1.08)`
  }
  // role2 0.52rem ~ -0.52rem
  // y = (-1.04 / window.innerWidth) * e.pageX + 0.52
  if (roleTwo.value) {
    roleTwo.value.style.transform = `translateX(${(-1.04 / window.innerWidth) * e.pageX + 0.52}rem) scale(1.13)`
  }
}
</script>
